...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	<h1 class="section-title">Crear cliente</h1>
	<a class="btn-ghost" href="{{ route('clientes.index') }}">Volver</a>
</div>

<div class="card-panel p-6 sm:p-8">
	<form class="space-y-4" method="POST" action="{{ route('clientes.store') }}">
		@csrf
		<div class="grid gap-4 sm:grid-cols-2">
			<div class="space-y-1">
				<label class="text-sm font-semibold text-slate-700">Nombres</label>
				<input name="Nombres" class="input-field @error('Nombres') border-red-300 ring-red-200 @enderror" value="{{ old('Nombres') }}" required>
			...
			<div class="space-y-1">
				...
				<input name="ApellidoP" class="input-field @error('ApellidoP') border-red-300 ring-red-200 @enderror" value="{{ old('ApellidoP') }}" required>
			...
			<div class="space-y-1">
				...
				<input name="ApellidoM" class="input-field @error('ApellidoM') border-red-300 ring-red-200 @enderror" value="{{ old('ApellidoM') }}">
			...
			<div class="space-y-1">
				...
				<input name="Segmento" class="input-field @error('Segmento') border-red-300 ring-red-200 @enderror" value="{{ old('Segmento') }}">
		...
		<div class="space-y-1">
			...
			<select name="ID_Canal" class="input-field @error('ID_Canal') border-red-300 ring-red-200 @enderror" required>
				...
				@foreach($canales as $canal)
					<option value="{{ $canal->ID_Canal }}" @selected(old('ID_Canal') == $canal->ID_Canal)>

...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	<h1 class="section-title">Editar ganancia #{{ $ganancia->Id_Informe_Ganancia }}</h1>
	<a class="btn-ghost" href="{{ route('ganancias.index') }}">Volver</a>
</div>

<div class="card-panel p-6 sm:p-8">
	<form class="space-y-4" method="POST" action="{{ route('ganancias.update', $ganancia->Id_Informe_Ganancia) }}">
		@csrf
		@method('PUT')
		<div class="grid gap-4 sm:grid-cols-2">
			<div class="space-y-1">
				<label class="text-sm font-semibold text-slate-700">PPTO (ganancia)</label>
				<input name="PPTO" type="number" step="0.01" class="input-field @error('PPTO') border-red-300 ring-red-200 @enderror" value="{{ old('PPTO', $ganancia->PPTO) }}" required>
			...
			<div class="space-y-1">
				...
				<input name="Periodo" class="input-field @error('Periodo') border-red-300 ring-red-200 @enderror" value="{{ old('Periodo', $ganancia->Periodo) }}" required>
			...
			<div class="space-y-1">
				...
				<select name="Mes" class="input-field @error('Mes') border-red-300 ring-red-200 @enderror" required>
					...
					@foreach([1=>'Enero',2=>'Febrero',3=>'Marzo',4=>'Abril',5=>'Mayo',6=>'Junio',7=>'Julio',8=>'Agosto',9=>'Septiembre',10=>'Octubre',11=>'Noviembre',12=>'Diciembre'] as $num=>$mes)
						<option value="{{ $num }}" @selected(old('Mes', $ganancia->Mes) == $num)>{{ $mes }}</option>
					@endforeach
				</select>
			...
			<div class="space-y-1">
				...
				<select name="ID_Sede" class="input-field @error('ID_Sede') border-red-300 ring-red-200 @enderror" required>
					...
					@foreach($sedes as $sede)
						<option value="{{ $sede->ID_Sede }}" @selected(old('ID_Sede', $ganancia->ID_Sede) == $sede->ID_Sede)>
		...
		<div class="pt-2">
			<button class="btn-primary" type="submit">Actualizar ganancia</button>
		</div>
	</form>
</div>
@endsection

...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	<h1 class="section-title">Editar vehículo #{{ $vehiculo->Id_Vehiculo }}</h1>
	<a class="btn-ghost" href="{{ route('vehiculos.index') }}">Volver</a>
</div>

<div class="card-panel p-6 sm:p-8">
	<form class="space-y-4" method="POST" action="{{ route('vehiculos.update', $vehiculo->Id_Vehiculo) }}">
		@csrf
		@method('PUT')
		<div class="grid gap-4 sm:grid-cols-2">
			<div class="space-y-1">
				<label class="text-sm font-semibold text-slate-700">Año</label>
				<input name="AnioVehiculo" type="number" class="input-field @error('AnioVehiculo') border-red-300 ring-red-200 @enderror" value="{{ old('AnioVehiculo', $vehiculo->AnioVehiculo) }}" required>
			...
			<div class="space-y-1">
				...
				<select name="Id_Modelo" class="input-field @error('Id_Modelo') border-red-300 ring-red-200 @enderror" required>
					...
					@foreach($modelos as $modelo)
						<option value="{{ $modelo->Id_Modelo }}" @selected(old('Id_Modelo', $vehiculo->Id_Modelo) == $modelo->Id_Modelo)>
			...
			<div class="space-y-1">
				...
				<select name="Id_Vendedor" class="input-field @error('Id_Vendedor') border-red-300 ring-red-200 @enderror" required>
					...
					@foreach($vendedores as $vendedor)
						<option value="{{ $vendedor->Id_Vendedor }}" @selected(old('Id_Vendedor', $vehiculo->Id_Vendedor) == $vendedor->Id_Vendedor)>
		...
		<div class="pt-2">
			<button class="btn-primary" type="submit">Actualizar vehículo</button>
		</div>
	</form>
</div>
@endsection

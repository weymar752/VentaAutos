...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	<h1 class="section-title">Crear usuario</h1>
	<a class="btn-ghost" href="{{ route('usuarios.index') }}">Volver</a>
</div>

<div class="card-panel p-6 sm:p-8">
	<form class="space-y-4" method="POST" action="{{ route('usuarios.store') }}">
		@csrf
		<div class="grid gap-4 sm:grid-cols-2">
			<div class="space-y-1">
				<label class="text-sm font-semibold text-slate-700">Nombre</label>
				<input name="name" class="input-field @error('name') border-red-300 ring-red-200 @enderror" value="{{ old('name') }}" required>
